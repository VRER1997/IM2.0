...
//    private byte[] yuv_Rotate90lr;
    ...
    public CameraHelper(EMCallManager.EMVideoCallHelper callHelper, SurfaceHolder localSurfaceHolder) {
...
//            yuv_Rotate90lr = new byte[mwidth * mheight * bitsperpixel / 8];
            ...
            EMClient.getInstance().callManager().getCallOptions().setVideoResolution(mwidth, mheight);
        ...
        if (start_flag == true) {
           ...
           /* if (isScreenOriatationPortrait()) {
                ...
                EMClient.getInstance().callManager().setCameraDataProcessor(new EMCallManager.EMCameraDataProcessor() {
                    @Override
                    public void onProcessData(byte[] bytes, Camera camera, int i, int i1, int i2) {
    ...
    boolean isScreenOriatationPortrait() {
        /*return MyApplication.getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT;*/
        return true;
    }
}
